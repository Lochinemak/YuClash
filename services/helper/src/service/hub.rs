...
const PROTOCOL_VERSION_HEADER: &str = "x-yuclash-helper-protocol";
...
mod tests {
    ...
    async fn start_rejects_a_caller_supplied_core_argument() {
        let response = warp::test::request()
            ...
            .body(
                r#"{"address":"\\\\.\\pipe\\YuClashCore_0123456789abcdef0123456789abcdef","sessionId":"0123456789abcdef0123456789abcdef","path":"attacker.exe"}"#,
            )
            .reply(&routes())
            .await;
    ...
    async fn start_releases_the_managed_core_before_rejecting_an_unverified_core() {
        ...
        *MANAGED_CORE.lock().unwrap() = Some(ManagedCore {
        ...
        let response = warp::test::request()
            ...
            .path("/start")
            .json(&StartParams {
                address: r"\\.\pipe\YuClashCore_0123456789abcdef0123456789abcdef".to_string(),
    ...
    fn verifies_core_sha256_in_all_build_modes() {
        let path =
            std::env::temp_dir().join(format!("yuclash-helper-core-sha256-{}", std::process::id()));
    ...
    fn only_accepts_random_core_pipe_namespace() {
        ...
        assert!(!is_allowed_core_pipe(r"\\.\pipe\YuClashCore"));
        assert!(!is_allowed_core_pipe(
