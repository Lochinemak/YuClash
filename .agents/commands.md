...
This fork's CI differs from upstream's. There is no parallel `dart`/`plugins`/
`go`/`android`/`rust` job split: one `test` job on `ubuntu-latest` runs every
non-build check, in this order:
...
cd plugins/setup/setup_hooks && dart pub get && dart analyze && dart test
go test . && go vet .                     # in core/
cargo fmt --check && cargo clippy && cargo test   # services/helper and plugins/rust_api/rust
```

A separate `windows-helper-test` job runs the helper's `windows-service`
feature tests on `windows-2022`. Both must pass before the `build` matrix
starts. Release builds run only for `v*` tag pushes; pull requests against
`main` run `test` and `build` but never `upload`.

Several gates upstream enforces exist here as scripts but are **not wired into
CI**, so running them is currently a manual step:
...
Treat all of these as advisory until the workflow adopts them; see the coverage
note in `.agents/rules.md` before assuming a green CI means the floors hold.

`bash tool/check_plugins.sh` is the plugin gate upstream's CI runs.
It discovers every `plugins/*/pubspec.yaml`, analyzes each package, and runs
`flutter test` wherever `test/*_test.dart` exists. This fork's workflow instead
enumerates four plugin packages by hand, so a new plugin package needs a
workflow edit here until `check_plugins.sh` replaces that loop.
