...
YuClash is a multi-platform proxy client based on FlClash and ClashMeta (mihomo), built with Flutter. It supports Android, Windows, macOS, and Linux, using a Material You design with Surfboard-like UI.
...
forks live under `chen08209`, the account that owns FlClash, which this project is
forked from — so unlike in FlClash itself, advancing one of these pins is not a
local decision here: the fork is maintained by the upstream author. What each fork
still waits on is the *upstream* fix that would let the pin be dropped entirely,
recorded below.
...
- This one is not optional while it lasts: this app depends on `win32_registry: ^3.0.3`
