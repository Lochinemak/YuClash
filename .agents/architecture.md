...
- Android may create an Always-on `VpnService` through `onStartCommand()` without YuClash's bound-service path. The service
...
- macOS: a standalone `YuClashCore`. `Release.xcconfig` pins release and profile `ARCHS` to the host because
...
- Linux and Windows: `YuClashCore`, the Rust `YuClashHelperService`, and a `manifest.json` holding `coreSha256`; the
...
- `/ping` is loopback-only and requires no request token. The Helper compares the requested SHA256 with its embedded value
  and checks that the fixed `YuClashCore.exe` beside it exists; `/start` performs the actual Core SHA256 verification before
...
- Flutter creates a 128-bit lowercase-hex session ID and uses it as the random named-pipe suffix. `/start` receives only
  that address and session ID, validates the fixed `YuClashCore_<session>` namespace, starts the fixed Core beside the
...
`core_name` and `helper_name` are load-bearing in this fork and must stay `YuClashCore`/`YuClashHelperService`. Nothing
resolves them at build time: the macOS `Stage Core` phase, `windows/CMakeLists.txt`, `linux/CMakeLists.txt`,
`windows/packaging/exe/inno_setup.iss`, `lib/common/path.dart`, `lib/common/constant.dart` and `services/helper` all
hardcode the `YuClash*` spelling. Taking upstream's `FlClash*` values during a sync breaks all six desktop jobs at the
packaging step while Android stays green, because Android's artifact name comes from `lib_name` and never goes through
`core_name`.

Architecture detection is automatic. The `--description` flag passed to `flutter_distributor` adds arch suffixes to artifact names, such as `YuClash-0.8.99-macos-arm64.dmg`.
...
- `YuClashHelperService.exe install` stops and removes any stale registration, creates the auto-start service for the
  current executable path, starts it, and waits for the running state.
- `YuClashHelperService.exe uninstall` stops the service, waits for shutdown, removes its registration, and is also used
...
- `YuClashHelperService install`, run through `pkexec` so polkit raises the system prompt, writes
...
- That ownership check is why the `flutter_distributor` fork normalizes the packaging tree to 0755/0644 before
  ...
  of 002 would otherwise ship `/opt/YuClash` as 0775, which the installer rejects as group-writable.
- The rpm spec sets `debug_package` and `__os_install_post` to nil for the same reason: rpmbuild's find-debuginfo and
  brp-strip rewrite `YuClashCore`, and a Core whose SHA256 no longer matches the Helper's embedded value is refused at
  `/start`, which silently degrades every launch to the direct Core.
- `YuClashHelperService uninstall` disables the unit, removes it and reloads systemd.
...
(`\\.\pipe\YuClashCore_<32 hex>` on Windows, `/tmp/YuClashSocket_<digits>.sock` on Linux), the session-scoped stop
