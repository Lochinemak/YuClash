...
- `core/Clash.Meta` is a fork of mihomo, and changes to it are budgeted for features, not repairs. Fixing a bug there is
  ...
  rebase. Solve it on the YuClash side of the boundary and note the mihomo behaviour you are working around. Reach into
  the submodule only for a feature that has nowhere else to live, or when the problem is one the YuClash patches
...
- `Co-authored-by` trailers crediting a coding agent are allowed here. Upstream forbids them and its
  `check_commit_msg.sh` rejects the known agent identities; this fork dropped that check deliberately, so do not
  reintroduce it from an upstream sync.
